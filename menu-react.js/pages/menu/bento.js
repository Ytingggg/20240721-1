import BentoTitle from '@/components/menu/bento-title'
import React from 'react'
import MenuNav from '@/components/menu/menu-nav'
import Image from 'next/image'
import { useState, useEffect, useRef } from 'react'
import { ImSpoonKnife } from 'react-icons/im'
import { MENU_BENTO } from '@/configs/api-path'
...
export default function Bento() {
  const [bento, setBento] = useState({ rows: [] })
  ...
  useEffect(() => {
    if (galleryRef.current && bento.rows.length > 0) {
      ...
      <style jsx>
        {`
          ...
          .size {
            ...
            display: flex;
            justify-content: center;
            background-image: url(/menuCard/menu-ben-L1.webp);
          ...
          @media (max-width: 1000px) {
            .size {
              ...
              display: flex;
              justify-content: center;
              background-image: url(/menuCard/menu-ben-m.webp);
              background-repeat: no-repeat;
              background-size: cover;
            }
          }
